...
#'
#' @keywords Log Periodic Power, LPPL
#'
#'
#'
#' @return
...
#'
#' @examples
#' Result <- Validate_LPPL(tradedate = ticker$Date
#'                        ,price = ticker$Close
#'                        ,mb=c(1:3)
#'                        ,MovingWindows=c(5,6)
#'                        ,gaRunCnt=100
  ...
  ,gaCrossover=0.8)
{
  ...
  Populations <- function(ip,lower,upper){
    ...
    colnames(InitialPopulation) <- c("A", "B","Tc","beta","C","omega","phi")
  ...
  for (i_mb in mb) {
    ...
    DT_END <- df[id_END,2]
    df_LogPrice <- subset(df,select=c("id","tradedate","logprice"),subset=id >= id_SRTPT & id <= id_END)
    ...
    for (i_mw in MovingWindows)
    {
      ...
      if(length(pk) > 2){
        lowerbounds <- c(A = max(LogPrice), B = -Inf, Tc = length(LogPrice), beta = 0.1, C = -1, omega = 5, phi = 0)
        upperbounds <- c(A = Inf, B = 0, Tc = Inf, beta = 0.9, C = 1,  omega = 15,  phi = 2*pi)
        Best <- data.frame(A=double(),B=double(),Tc=double(),beta=double(),C=double(),omega=double(),phi=double(),RMSE=double(), MW=integer())
        {
          if(ParallelMode){
            ...
            Best <- foreach(icount(gaRunCnt), .combine=rbind, .packages = "GA" ) %dopar%
              {
                trycount <- 0
                while(TRUE)
                {
